...
import { parseUserPlaceId } from '@/menu/utils/place-id.util';
import {
  SCHEDULER_LOCKS,
  BATCH_CONFIG,
...
export class RatingSchedulerService implements OnModuleInit {
        ...
        try {
          await this.dataSource.transaction(async (manager) => {
            ...
            const {
              ...
              idPlaceholders,
              parameters,
            } = this.buildBulkUpdateClauses(updates);
  ...
  /**
   * CASE 절 파라미터 빌드 헬퍼
   * avgRating과 ratingCount에 대한 CASE 절 및 IN 절 파라미터를 생성합니다.
   */
  private buildBulkUpdateClauses(
    updates: Array<{ id: number; avgRating: number; ratingCount: number }>,
  ): {
    avgRatingCases: string;
    ratingCountCases: string;
    ...
    return { avgRatingCases, ratingCountCases, idPlaceholders, parameters };
  }
}
