...
describe('PreferenceUpdateAiService', () => {
  ...
  describe('generatePreferenceAnalysis', () => {
    // Note: onModuleInit() is not called in a beforeEach to avoid conflicts
    // Each test that needs OpenAI should either call service.onModuleInit() or mock service['openai'] directly

    it('should generate preference analysis successfully', async () => {
      // Setup: Initialize service (calls onModuleInit)
      service.onModuleInit();
      ...
      const mockOpenAI: MockOpenAI = createMockOpenAI();
      mockOpenAI.chat.completions.create.mockResolvedValue(
        mockResponse as never,
      );
      service['openai'] = mockOpenAI as never;
    ...
    it('should throw ExternalApiException on OpenAI API error', async () => {
      ...
      await expect(
        service.generatePreferenceAnalysis(current, slotMenus),
      ).rejects.toThrow('Failed to generate preference analysis.');
    });
  ...
  describe('validateSchema', () => {
    // Note: onModuleInit() is not called in beforeEach to avoid conflicts
    // These tests are for private method validation, no OpenAI client needed

    it('should throw OpenAIResponseException when data is null', () => {
      // Setup: Initialize service
      service.onModuleInit();
      // Act & Assert
      expect(() => service['validateSchema'](null as never)).toThrow(
    ...
    it('should throw OpenAIResponseException when data is undefined', () => {
      ...
      // Act & Assert
      expect(() => service['validateSchema'](undefined as never)).toThrow(
        OpenAIResponseException,
      );
      expect(() => service['validateSchema'](undefined as never)).toThrow(
        'Invalid response format.',
