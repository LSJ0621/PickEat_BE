...
describe('NotificationService', () => {
  ...
  describe('findPublished', () => {
    it('should return only PUBLISHED notifications', async () => {
      const queryDto: NotificationListQueryDto = {};
      const publishedNotifications = [
        { ...mockNotification, status: NotificationStatus.PUBLISHED },
      ];
      ...
      const mockQb = {
        ...
        getManyAndCount: jest
          .fn()
          .mockResolvedValue([publishedNotifications, 1]),
      };

      repository.createQueryBuilder = jest.fn().mockReturnValue(mockQb) as any;

      await service.findPublished(queryDto);
    ...
    it('should order pinned items first then by publishedAt desc', async () => {
      const queryDto: NotificationListQueryDto = {};
    ...
    it('should return paginated results', async () => {
      const queryDto: NotificationListQueryDto = { page: 2, limit: 10 };
      ...
      const result = await service.findPublished(queryDto);
  ...
  describe('findOnePublished', () => {
    it('should return notification when PUBLISHED notification exists', async () => {
      const publishedNotification = {
        ...mockNotification,
        status: NotificationStatus.PUBLISHED,
      };

      repository.findOne.mockResolvedValue(publishedNotification);

      const result = await service.findOnePublished(1);

      expect(repository.findOne).toHaveBeenCalledWith({
        where: { id: 1, status: NotificationStatus.PUBLISHED },
      });
      expect(result).toEqual(publishedNotification);
    });

    it('should throw NotFoundException when notification is not PUBLISHED', async () => {
      repository.findOne.mockResolvedValue(null);
    ...
    it('should throw NotFoundException when notification does not exist', async () => {
      ...
      await expect(service.findOnePublished(999)).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.findOnePublished(999)).rejects.toThrow(
        '공지사항을 찾을 수 없습니다. (ID: 999)',
  ...
  describe('incrementViewCount', () => {
    ...
    it('should call increment atomically for multiple views', async () => {
      await service.incrementViewCount(1);
      await service.incrementViewCount(1);
      await service.incrementViewCount(1);
